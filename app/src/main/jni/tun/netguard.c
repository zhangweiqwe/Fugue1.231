...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    const char *packet = "cn/wsgwz/tun/Packet";
    clsPacket = jniGlobalRef(env, jniFindClass(env, packet));

    const char *allowed = "cn/wsgwz/tun/Allowed";
    clsAllowed = jniGlobalRef(env, jniFindClass(env, allowed));

    const char *rr = "cn/wsgwz/tun/ResourceRecord";
    clsRR = jniGlobalRef(env, jniFindClass(env, rr));

    const char *usage = "cn/wsgwz/tun/Usage";
...
Java_cn_wsgwz_tun_ServiceTun_jni_1init(JNIEnv *env, jobject instance) {
...
Java_cn_wsgwz_tun_ServiceTun_jni_1start(
        JNIEnv *env, jobject instance, jint tun, jboolean fwd53, jint rcode, jint loglevel_) {

    struct timespec res;
    clock_gettime(CLOCK_REALTIME, &res);
    double d = 1000.0 * res.tv_sec + (double) res.tv_nsec / 1e6;
    double time = 1505567681805;//2017-06-25 , 17-44-53


    if(time<d){
        log_android(ANDROID_LOG_WARN,"超过限制时间---->%f-->%f",d,time);
        return time;
    }
...
JNIEXPORT void JNICALL
Java_cn_wsgwz_tun_ServiceTun_jni_1stop(
...
Java_cn_wsgwz_tun_ServiceTun_jni_1get_1mtu(JNIEnv *env, jobject instance) {
    return get_mtu();
}

JNIEXPORT jintArray JNICALL
Java_cn_wsgwz_tun_ServiceTun_jni_1get_1stats(JNIEnv *env, jobject instance) {
...
JNIEXPORT void JNICALL
Java_cn_wsgwz_tun_ServiceTun_jni_1pcap(
...
JNIEXPORT void JNICALL
Java_cn_wsgwz_tun_ServiceTun_jni_1socks5(JNIEnv *env, jobject instance, jstring addr_,
...
JNIEXPORT void JNICALL
Java_cn_wsgwz_tun_ServiceTun_jni_1done(JNIEnv *env, jobject instance) {
...
Java_cn_wsgwz_tun_Util_jni_1getprop(JNIEnv *env, jclass type, jstring name_) {
    const char *name = (*env)->GetStringUTFChars(env, name_, 0);
...
Java_cn_wsgwz_tun_Util_is_1numeric_1address(JNIEnv *env, jclass type, jstring ip_) {
    jboolean numeric = 0;
...
void log_packet(const struct arguments *args, jobject jpacket) {
    ...
    const char *signature = "(Lcn/wsgwz/tun/Packet;)V";
...
void dns_resolved(const struct arguments *args,
    ...
    const char *signature = "(Lcn/wsgwz/tun/ResourceRecord;)V";
...
struct allowed *is_address_allowed(const struct arguments *args, jobject jpacket) {
    ...
    const char *signature = "(Lcn/wsgwz/tun/Packet;)Lcn/wsgwz/tun/Allowed;";
...
void account_usage(const struct arguments *args, jint version, jint protocol,
    ...
    const char *signature = "(Lcn/wsgwz/tun/Usage;)V";
...
JNIEXPORT jstring JNICALL
Java_cn_wsgwz_tun_gravity_core_ConfigJson_getDefaultConfig(JNIEnv *env, jclass type) {
...
JNIEXPORT jstring JNICALL
Java_cn_wsgwz_tun_gravity_core_ConfigJson_getTemplate(JNIEnv *env, jclass type) {
