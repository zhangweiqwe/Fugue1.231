...
public class Util {
    ...
    public static List<String> getDefaultDNS(Context context) {
        ...
        listDns.add(TextUtils.isEmpty(dns1) ? "114.114.114.114" : dns1);
        listDns.add(TextUtils.isEmpty(dns2) ? "114.114.115.115" : dns2);
        return listDns;
    }
    ...
    public static boolean setBackground(Activity activity){
        ...
        SharedPreferences prfs = PreferenceManager.getDefaultSharedPreferences(activity);
        String bgPath = prfs.getString(Const.Prefs.KEY_BG,null);
        if(bgPath==null){return false;}
        Uri uri = Uri.fromFile(new File(bgPath));
        if(uri==null){return false;}
        Drawable drawable = null;
        try {
            drawable = Drawable.createFromStream(activity.getContentResolver().openInputStream(uri),null);
    ...
    public static boolean setBackground(View view){
        if(view==null){return false;}
        SharedPreferences prfs = PreferenceManager.getDefaultSharedPreferences(view.getContext());
        String bgPath = prfs.getString(Const.Prefs.KEY_BG,null);
        if(bgPath==null){return false;}
        Uri uri = Uri.fromFile(new File(bgPath));
        if(uri==null){return false;}
        Drawable drawable = null;
        try {
            drawable = Drawable.createFromStream(view.getContext().getContentResolver().openInputStream(uri),null);
    ...
    public static void setTheme(Activity activity){
        ...
        //boolean isSetBgOk = Util.setBackground(activity);

        if(activity instanceof ActivityMain){
            /*if(!isSetBgOk){
                final boolean trasparent = prefs.getBoolean(Const.Prefs.KEY_ActivityMain_TRANPARENT, Const.Prefs.DEFAUL_VALUE_ActivityMain_IS_TRANPARENT);
                if (trasparent) {
                    activity.setTheme(R.style.ThemeMainActivity);
                } else {
                    activity.setTheme(R.style.ThemeMainActivity2);
                }
            }else {
                activity.setTheme(R.style.ThemeMainActivity3);
            }*/
        }else if(activity instanceof SlidingAroundBaseActivity){
            /*if(!isSetBgOk){
                final boolean trasparent = prefs.getBoolean(Const.Prefs.KEY_SlidingAroundBaseActivity_TRANPARENT,Const.Prefs.DEFAUL_VALUE_SlidingAroundBaseActivity_IS_TRANPARENT);
                if(trasparent){
