...
public class ProxyTaskZ implements Runnable {
    public static final String LOG_TAG = ProxyTaskZ.class.getSimpleName();
    //5, 2, 0, 2
    ...
    public ProxyTaskZ(Socket socket, ServiceTun serviceTun, Config config) {
        this.socket = socket;
    ...
    public void run() {
        ...
        SocketThreadOutputZ out = null;
        SocketThreadInputZ in = null;
        try {
            ...
            if (a_in.read() == 0x5) {
                ...
                // 交换流数据
                out = new SocketThreadOutputZ(a_in, b_out, config, socketD);
                out.start();
                in = new SocketThreadInputZ(b_in, a_out);
                in.start();
                out.join();
                in.join();
